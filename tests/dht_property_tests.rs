...
fn arb_record_with_expiration() -> impl Strategy<Value = Record> {
    (arb_record(), any::<bool>())
        .prop_map(|(mut record, should_expire)| {
            if should_expire {
        ...
        rt.block_on(async {
            ...
            for (publisher, expected_keys) in publishers_to_keys {
                ...
                for record in &indexed_records {
                    prop_assert_eq!(record.publisher.to_string(), publisher);
...
proptest! {
    ...
    fn prop_store_get_consistency(
        ...
        rt.block_on(async {
            let config = DHTConfig::default();
            let storage = OptimizedDHTStorage::with_cache_size(config, 100); // Large enough cache
            ...
            for record in &records {
                ...
                prop_assert_eq!(retrieved.value, record.value);
...
proptest! {
    ...
    fn prop_lru_ordering_maintained(
        ...
        rt.block_on(async {
            let config = DHTConfig::default();
            let storage = OptimizedDHTStorage::with_cache_size(config, 10); // Small cache
...
mod deterministic_properties {
    ...
    async fn test_property_examples() -> Result<()> {
        ...
        for i in 0..20 {
            ...
            let stats = storage.get_stats().await;
            assert!(stats.total_records <= 5, "Property violation: cache bounds exceeded");
        }
        ...
        assert!(retrieved.is_none(), "Property violation: expired record returned");
