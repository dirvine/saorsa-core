...
use saorsa_rsps::{RootCid, Cid, WitnessReceipt};
use std::sync::Arc;
use std::time::Duration;
use tokio::time;

/// Test configuration for RSPS
fn test_config() -> RspsConfig { RspsConfig::default() }

/// Create a test storage instance
async fn create_test_storage() -> RspsDhtStorage {
    let base_storage = Arc::new(OptimizedDHTStorage::new(saorsa_core::dht::DHTConfig::default()));
    let local_peer = "peer-1".to_string();
    let config = test_config();
    RspsDhtStorage::new(base_storage, local_peer, config).await.unwrap()
}
...
let root_cid = RootCid::from(blake3::hash(b"test-root-cid").as_bytes().to_owned());
    ...
    // Store provider
    storage
        .store_provider(root_cid.clone(), provider.clone(), vec![], saorsa_rsps::Rsps::new(root_cid.clone(), 0, &[], &saorsa_rsps::RspsConfig::default()).unwrap())
        .await?;
...
async fn test_cache_admission_control() -> Result<()> {
   ...
   let excluded_root = RootCid::from(blake3::hash(b"excluded-root").as_bytes().to_owned());
    ...
    let provider = "provider-1".to_string();
    storage
        .store_provider(included_root.clone(), provider.clone(), vec![], saorsa_rsps::Rsps::new(included_root.clone(), 0, &[], &saorsa_rsps::RspsConfig::default()).unwrap())
        .await?;
   ...
   let included_cid = Cid::from(blake3::hash(b"included-content").as_bytes().to_owned());
    assert!(storage.cache_if_allowed(included_root.clone(), included_cid, included_content.clone()).await?);

   let excluded_cid = Cid::from(blake3::hash(b"excluded-content").as_bytes().to_owned());
    assert!(!storage.cache_if_allowed(excluded_root.clone(), excluded_cid, excluded_content.clone()).await?);
...
async fn test_witness_receipt_generation() -> Result<()> {
   ...
   let content_cid = Cid::from(blake3::hash(b"test-content").as_bytes().to_owned());
    ...
    assert!(!receipt.witness_pseudonym.proof.is_empty() || !receipt.witness_pseudonym.value.is_empty());
...
async fn test_rsps_update_with_new_content() -> Result<()> {
   ...
   let root_cid = RootCid::from(blake3::hash(b"evolving-root").as_bytes().to_owned());
    ...
    for content in &content_items {
        ...
        storage.cache_if_allowed(root_cid.clone(), cid, content.clone()).await?;
...
async fn test_cache_eviction_respects_rsps() -> Result<()> {
    ...
    for i in 0..150 {
        ...
        if i % 2 == 0 {
            ...
            let _ = storage.cache_if_allowed(root_cid.clone(), content, vec![]).await?;
        }
    }
