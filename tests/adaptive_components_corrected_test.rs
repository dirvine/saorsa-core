...
use saorsa_core::adaptive::{
    eviction::{CacheState, LRUStrategy, LFUStrategy, EvictionStrategy, EvictionStrategyType},
    learning::{
        ThompsonSampling, QLearnCacheManager, ContentType, StrategyChoice, 
        NodeFeatures, ChurnPredictor, NodeEvent
    },
    multi_armed_bandit::{MultiArmedBandit, MABConfig, RouteId, RouteDecision},
    replication::{ReplicationManager, ReplicationStrategy, ReplicaInfo},
    security::{SecurityManager, SecurityConfig, BlacklistReason, SecurityError},
    ...
    q_learning_cache::{StateVector, AccessInfo, CacheAction},
    identity::NodeIdentity,
    NodeId, ContentHash, Outcome, NetworkConditions, LearningContext
};
use std::{
...
async fn test_thompson_sampling_real_api() -> anyhow::Result<()> {
    ...
    for content_type in &content_types {
        for _ in 0..50 {
            ...
            *strategy_counts.entry((content_type, strategy)).or_insert(0) += 1;
            ...
            let latency = if success { 50 + rand::random::<u64>() % 100 } else { 500 + rand::random::<u64>() % 1000 };
    ...
    for content_type in &content_types {
        let (lower, upper) = ts.get_confidence_interval(*content_type, StrategyChoice::Kademlia).await;
        println!("Confidence interval for {:?} with Kademlia: [{:.3}, {:.3}]", content_type, lower, upper);
        assert!(lower >= 0.0 && upper <= 1.0 && lower <= upper, "Invalid confidence interval");
    }
...
async fn test_multi_armed_bandit_real_api() -> anyhow::Result<()> {
    ...
    for _ in 0..100 {
        ...
        let decision = mab.select_route(destination, content_type, &strategies).await?;
        println!("Selected route: {:?} with probability {:.3}", decision.route_id.strategy, decision.probability);
        ...
        let outcome = Outcome {
            success,
            latency_ms: if success { 50 + rand::random::<u64>() % 100 } else { 500 + rand::random::<u64>() % 1000 },
            hops: rand::random::<usize>() % 6 + 1,
        };
        
        mab.update_route(&decision.route_id, content_type, &outcome).await?;
        
        *route_successes.entry(decision.route_id.strategy).or_insert((0, 0)) = {
            let (s, t) = route_successes.get(&decision.route_id.strategy).unwrap_or(&(0, 0));
    ...
    for (strategy, (successes, total)) in &route_successes {
        ...
        println!("  {:?}: {:.1}% ({}/{})", strategy, rate * 100.0, successes, total);
    }
    
    let metrics = mab.get_metrics().await;
    println!("MAB metrics: {:?}", metrics);
    assert!(metrics.total_decisions > 0, "Should have made routing decisions");
...
async fn test_security_manager_real_api() -> anyhow::Result<()> {
    ...
    // Test blacklisting
    security.blacklist_node(test_node.id.clone(), BlacklistReason::Manual("Test blacklist".to_string())).await;
    ...
    assert!(metrics.blacklisted_nodes > 0, "Should have blacklisted nodes");
...
async fn test_replication_manager_real_api() -> anyhow::Result<()> {
    ...
    let replica_info = replication.replicate_content(&content_hash, content, metadata).await?;
    println!("Replica info: {:?}", replica_info);
    assert!(!replica_info.storing_nodes.is_empty(), "Should have storing nodes");
...
async fn test_eviction_strategies_real_api() -> anyhow::Result<()> {
    ...
    for (i, hash) in content_hashes.iter().enumerate() {
        access_info.insert(*hash, AccessInfo {
            access_count: (i + 1) as u64 * 10,
            last_access: SystemTime::now().duration_since(UNIX_EPOCH)?.as_secs() - (i as u64 * 60),
            size: 100,
            content_type: ContentType::DataRetrieval,
        });
    }
...
async fn test_q_learning_cache_real_api() -> anyhow::Result<()> {
    ...
    for (hash, data) in content_hashes.iter().zip(test_content.iter()) {
        ...
        println!("Q-learning decided action for content {}: {:?}", hash.0[0], action);
        
        // Test action execution
        manager.execute_action(hash, action, Some(data.clone())).await?;
    }
    
    // Test caching decisions
    let new_hash = ContentHash([4u8; 32]);
    let new_data = vec![4u8; 100];
    manager.decide_caching(new_hash, new_data, ContentType::DataRetrieval).await?;
    
    // Get cache statistics
    let stats = manager.get_stats_async().await;
    println!("Cache stats: hits={}, misses={}, size={}, items={}, hit_rate={:.2}%", 
             stats.hits, stats.misses, stats.size_bytes, stats.item_count, stats.hit_rate * 100.0);
...
async fn test_churn_predictor_real_api() -> anyhow::Result<()> {
    ...
    println!("Churn prediction for stable node: 1h={:.1}%, 6h={:.1}%, 24h={:.1}%, confidence={:.1}%",
    ...
    predictor.record_node_event(&unstable_node, NodeEvent::Connected).await?;
    predictor.update_node_behavior(&unstable_node, unstable_features).await?;
    
    let unstable_prediction = predictor.predict(&unstable_node).await;
    println!("Churn prediction for unstable node: 1h={:.1}%, 6h={:.1}%, 24h={:.1}%, confidence={:.1}%",
    ...
    println!("Should replicate from stable node: {}", should_replicate_stable);
    println!("Should replicate from unstable node: {}", should_replicate_unstable);
    
    assert!(prediction.probability_1h <= 1.0, "Probabilities should be <= 1.0");
    assert!(unstable_prediction.probability_1h <= 1.0, "Probabilities should be <= 1.0");
    
    // Record node disconnection
    predictor.record_node_event(&unstable_node, NodeEvent::Disconnected).await?;
...
async fn test_integrated_adaptive_system() -> anyhow::Result<()> {
    ...
    while start_time.elapsed() < Duration::from_secs(1) && total_operations < 50 {
        ...
        let latency = if success { 50 + rand::random::<u64>() % 100 } else { 500 };
        thompson.update(content_type, strategy, success, latency).await?;
        
        // MAB for route selection
        let destination = NodeId { hash: [rand::random::<u8>(); 32] };
        let strategies = vec![StrategyChoice::Kademlia, StrategyChoice::Hyperbolic];
        let route_decision = mab.select_route(&destination, content_type, &strategies).await?;
        ...
        mab.update_route(&route_decision.route_id, content_type, &outcome).await?;
        
        // Cache operations
        let content_hash = ContentHash([rand::random::<u8>(); 32]);
        let data = vec![rand::random::<u8>(); 100 + rand::random::<usize>() % 400];
        cache_manager.decide_caching(content_hash, data, content_type).await?;
        ...
        predictor.update_node_behavior(&destination, node_features).await?;
    ...
    println!("Completed {} integrated operations in {:?}", total_operations, start_time.elapsed());
    ...
    assert!(ts_metrics.total_decisions > 0, "Thompson Sampling should make decisions");
