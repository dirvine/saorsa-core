...
impl EnhancedSignatureVerifier {
    ...
    pub fn verify_batch(
        &mut self,
        requests: Vec<BatchVerificationRequest>,
        ...
        if requests.len() > MAX_BATCH_SIZE {
            ...
            return requests
                .into_iter()
                .enumerate()
                .map(|(index, _req)| BatchVerificationResult {
                    index,
                    success: false,
    ...
    fn verify_single_batch_request(&mut self, request: BatchVerificationRequest) -> Result<()> {
        ...
        // Perform constant-time signature verification
        self.verify_signature_constant_time(&cached_key.key, &request.message, &request.signature)
    }
    ...
    fn validate_timestamp_freshness(&self, record: &PeerDHTRecord) -> Result<()> {
        ...
        if age > SIGNATURE_FRESHNESS_WINDOW.as_secs() {
            return Err(P2PError::Security(
                crate::error::SecurityError::SignatureVerificationFailed(
    ...
    fn get_or_create_cached_key(
        &mut self,
        public_key: &VerifyingKey,
    ) -> Result<CachedVerificationKey> {
    ...
    fn create_verifying_key(&self, public_key: &VerifyingKey) -> Result<VerifyingKey> {
        ...
        if self.is_low_order_point(key_bytes) {
            return Err(P2PError::Security(crate::error::SecurityError::InvalidKey(
                "Invalid curve point".to_string().into(),
            )));
        }
    ...
    fn verify_signature_constant_time(
        ...
        verifying_key.verify(message, signature).map_err(|_| {
            P2PError::Security(crate::error::SecurityError::SignatureVerificationFailed(
                "Signature verification failed".to_string().into(),
            ))
        })
    }
    ...
    fn update_stats(&mut self, start_time: Instant, success: bool) {
        ...
        self.stats.avg_verification_time_us =
            (self.stats.avg_verification_time_us * self.stats.total_verified + elapsed)
                / total_verifications;
...
mod tests {
    ...
    use crate::peer_record::{EndpointId, NatType, PeerDHTRecord, PeerEndpoint, UserId};
    ...
    fn test_timestamp_freshness_validation() {
        ...
        record.timestamp = std::time::SystemTime::now()
            ...
            .expect("valid crypto operation")
            .as_secs()
            - 400; // 400 seconds ago (older than 5 minutes)
