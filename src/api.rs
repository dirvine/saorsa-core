...
pub async fn create_entity(
    ...
    // Convert four words to owned strings
    let words = [
        four_words[0].to_string(),
        four_words[1].to_string(),
        four_words[2].to_string(),
        four_words[3].to_string(),
    ];

    // Validate four-word address
    if !fw_check(words.clone()) {
        anyhow::bail!("Invalid four-word address");
    }

    // Create entity
    let mut entity = internal_create_entity(entity_type, name.to_string(), words).await?;

    // Apply optional settings
    if let Some(desc) = description {
        entity.core.metadata.description = Some(desc);
    }
    if let Some(custom_settings) = settings {
        entity.core.metadata.settings = custom_settings;
    }

    // Register entity
    ENTITY_REGISTRY.register(entity).await
}
...
///
/// # Returns
/// * `Option<Arc<Entity>>` - The entity if found
pub async fn get_entity(id: &EntityId) -> Option<Arc<Entity>> {
    ENTITY_REGISTRY.get(id).await
}

/// Get an entity by its four-word address
///
/// # Arguments
/// * `four_words` - Four-word address array
///
/// # Returns
/// * `Option<Arc<Entity>>` - The entity if found
pub async fn get_entity_by_address(four_words: [&str; 4]) -> Option<Arc<Entity>> {
    let words = [
        four_words[0].to_string(),
        four_words[1].to_string(),
        four_words[2].to_string(),
    ...
    match FourWordAddress::from_words(words) {
        Ok(address) => ENTITY_REGISTRY.get_by_address(&address).await,
        Err(_) => None,
    }
}
...
///
/// # Returns
/// * `Vec<EntityInfo>` - List of entity information
pub async fn list_entities() -> Vec<EntityInfo> {
    ENTITY_REGISTRY.list().await
}
...
pub async fn entity_disk_write(
    ...
    disk_write(disk, path, &content, metadata).await?;
    Ok(())
}
...
pub async fn entity_disk_read(
    entity_handle: &EntityHandle,
    path: &str,
    is_public: bool,
) -> Result<Vec<u8>> {
    use crate::virtual_disk::disk_read;
...
/// * `is_public` - Whether to list public or private disk
///
/// # Returns
/// * `Result<Vec<FileEntry>>` - List of files
pub async fn entity_disk_list(
    entity_handle: &EntityHandle,
    path: &str,
    recursive: bool,
    is_public: bool,
) -> Result<Vec<crate::virtual_disk::FileEntry>> {
    use crate::virtual_disk::disk_list;
    ...
    disk_list(disk, path, recursive).await
}

/// Enable or disable website publishing for an entity
///
/// # Arguments
/// * `entity_handle` - Handle to the entity
/// * `enabled` - Whether to enable website publishing
///
/// # Returns
/// * `Result<()>` - Success or error
...
/// Get the website URL for an entity (if enabled)
///
/// # Arguments
/// * `entity_handle` - Handle to the entity
///
/// # Returns
/// * `Option<String>` - The website URL if enabled
pub fn entity_website_url(entity_handle: &EntityHandle) -> Option<String> {
    entity_handle.website_url()
}
