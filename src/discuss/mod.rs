...
impl DiscussManager {
    ...
    pub async fn create_category(
        ...
        self.storage
            .store_encrypted(&key, &category, ttl::PROFILE, None)
            .await?;
    ...
    pub async fn create_topic(
        ...
        self.storage
            .store_encrypted(&key, &topic, ttl::PROFILE, None)
            .await?;
    ...
    pub async fn post_reply(
        ...
        if !matches!(topic.status, TopicStatus::Open) {
            return Err(DiscussError::InvalidOperation(
                "Topic is not open".to_string(),
            ));
        }
        ...
        self.storage
            .store_encrypted(&key, &reply, ttl::MESSAGE, None)
            .await?;
    ...
    /// Vote on content
    pub async fn vote(&mut self, target: VoteTarget, vote_type: VoteType) -> Result<()> {
        match target {
            ...
            VoteTarget::Reply(topic_id, reply_id) => {
                ...
                // Recalculate score
                reply.votes.upvotes = reply
                    .votes
                    .voters
                    .values()
                    .filter(|&&v| matches!(v, VoteType::Up))
                    .count() as u64;
                reply.votes.downvotes = reply
                    .votes
                    .voters
                    .values()
                    .filter(|&&v| matches!(v, VoteType::Down))
                    .count() as u64;
    ...
    pub async fn edit_topic(
        ...
        if !can_edit {
            return Err(DiscussError::PermissionDenied(
                "Cannot edit topic".to_string(),
            ));
        }
        ...
        match &category.access_level {
            ...
            AccessLevel::Announcement => {
                if write {
                    ...
                    Err(DiscussError::PermissionDenied(
                        "Announcement category".to_string(),
                    ))
                } else {
                    Ok(())
                }
