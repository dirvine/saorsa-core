...
use super::types::*;
use super::signaling::{SignalingHandler, SignalingEvent, SignalingState};
use super::media::{MediaStreamManager, MediaEvent};
...
impl CallManager {
    ...
    pub async fn initiate_call(
        ...
        // Create media stream
        let stream = self.media.create_stream(call_id, constraints.clone()).await?;
    ...
    pub async fn create_multi_party_call(
        ...
        info!("Created multi-party call {} with {} participants using {:?} architecture",
              call_id.0, multi_call.participants.len(), architecture);
    ...
    pub async fn request_recording_consent(&self, consent: RecordingConsent) -> Result<()> {
        info!("Requesting recording consent for call {}", consent.call_id.0);
    ...
    pub async fn grant_recording_consent(&self, call_id: CallId, _participant: FourWordAddress) -> Result<()> {
    ...
    async fn cleanup_task(calls: Arc<RwLock<HashMap<CallId, CallSession>>>) {
        ...
        loop {
            ...
            calls.retain(|_, session| {
                let age = now.signed_duration_since(session.start_time.unwrap_or(session.created_at));
                ...
                if session.state == CallState::Failed {
                    let failed_age = now.signed_duration_since(session.end_time.unwrap_or(session.created_at));
