...
impl SignalingHandler {
    ...
    pub fn new(local_identity: FourWordAddress, dht_client: Arc<RwLock<DhtCoreEngine>>) -> Self {
    ...
    pub fn validate_sdp(sdp: &str) -> Result<()> {
        ...
        for field in &required_fields {
            if !sdp.contains(field) {
                return Err(anyhow::anyhow!(
                    "Invalid SDP: missing required field {}",
                    field
                ));
            }
        }
    ...
    fn sanitize_sdp_content(sdp: &str) -> Result<()> {
        ...
        for line in lines {
            ...
            {
                return Err(anyhow::anyhow!(
                    "Private IP address not allowed in SDP connection data: {}",
                    line
                ));
            }
            ...
            if line.contains("a=tool:") && line.to_lowercase().contains("script") {
                return Err(anyhow::anyhow!(
                    "Potentially dangerous SDP attribute detected: {}",
                    line
                ));
            }
        }
    ...
    pub fn validate_quic_endpoint(endpoint: &str) -> Result<()> {
        ...
        if !endpoint.contains(':') {
            return Err(anyhow::anyhow!(
                "Invalid QUIC endpoint format: missing port"
            ));
        }
    ...
    pub async fn send_offer(&self, offer: CallOffer) -> Result<()> {
        ...
        Self::validate_sdp(&offer.sdp).context("Invalid SDP in call offer")?;
    ...
    pub async fn send_answer(&self, answer: CallAnswer) -> Result<()> {
        ...
        if answer.accepted {
            Self::validate_sdp(&answer.sdp).context("Invalid SDP in call answer")?;
        }
        ...
        if let Some(session) = sessions.get_mut(&answer.call_id) {
            ...
            // Send via DHT
            self.send_signaling_message(&session.remote_peer, message)
                .await?;
            ...
            info!(
                ...
                answer.call_id.0, answer.accepted
            );
        } else {
            return Err(anyhow::anyhow!(
                "No active session for call {}",
                answer.call_id.0
            ));
        }
    ...
    pub async fn establish_quic_connection(&self, call_id: CallId) -> Result<()> {
        ...
        if let Some(session) = sessions.get_mut(&call_id) {
            ...
            if self.quic_config.hole_punching {
                debug!(
                    "Using coordinator-based hole punching for peer {}",
                    session.remote_peer
                );
    ...
    pub async fn end_call(&self, call_id: CallId) -> Result<()> {
        ...
        if let Some(session) = sessions.get(&call_id) {
            ...
            let _ = self
                .event_sender
                .send(SignalingEvent::CallEnded { call_id });
    ...
    pub async fn reject_call(&self, call_id: CallId) -> Result<()> {
        ...
        if let Some(session) = sessions.get(&call_id) {
            ...
            let _ = self
                .event_sender
                .send(SignalingEvent::CallRejected { call_id });
    ...
    pub fn set_quic_config(&mut self, config: NativeQuicConfiguration) {
        info!(
            "Updated native QUIC configuration: dht_discovery={}, hole_punching={}",
    ...
    async fn send_signaling_message(
        ...
        let _data =
            serde_json::to_vec(&envelope).context("Failed to serialize signaling message")?;
    ...
    async fn _handle_incoming_message(&self, envelope: SignalingEnvelope) -> Result<()> {
        ...
        if !self._verify_message_signature(&envelope) {
            warn!(
                "Invalid signature on signaling message from {}",
                envelope.sender
            );
            return Ok(());
        }
    ...
    async fn _handle_incoming_offer(&self, offer: CallOffer) -> Result<()> {
        ...
        let _ = self
            .event_sender
            .send(SignalingEvent::IncomingCall { offer });
    ...
    async fn _handle_incoming_answer(&self, answer: CallAnswer) -> Result<()> {
        ...
        if let Some(session) = sessions.get_mut(&answer.call_id) {
            ...
            // Emit event
            let _ = self.event_sender.send(SignalingEvent::AnswerReceived {
                answer: answer.clone(),
            });

            // Start QUIC connection establishment
            if answer.accepted {
                self._handle_quic_connection_establishment(answer.call_id)
                    .await?;
            }
        }
    ...
    async fn _handle_quic_connection_establishment(&self, call_id: CallId) -> Result<()> {
        ...
        if let Some(session) = sessions.get_mut(&call_id) {
            ...
            let _ = self
                .event_sender
                .send(SignalingEvent::ConnectionEstablishing { call_id });
        }
