...
mod tests {
        ...
        if let Ok(event) = alice_events.try_recv() {
            match event {
                WebRtcEvent::Call(CallEvent::CallInitiated {
                    ...
                    assert_eq!(
                        callee,
                        crate::messaging::user_handle::UserHandle::from(bob.to_string())
                    );
    ...
    async fn test_call_rejection_flow() {
        ...
        if let Ok(event) = alice_events.try_recv() {
            match event {
                WebRtcEvent::Call(CallEvent::CallRejected {
                    call_id: rejected_id,
                }) => {
    ...
    async fn test_call_manager_state_transitions() {
        ...
        assert!(
            session
                .participants
                .contains(&crate::messaging::user_handle::UserHandle::from(
                    bob.to_string()
                ))
        );
    ...
    async fn test_call_timeout_handling() {
        ...
        let age = chrono::Utc::now()
            .signed_duration_since(session.start_time.unwrap_or(session.created_at));
