...
impl MediaStreamManager {
    ...
    pub async fn create_stream(
        ...
        info!(
            "Creating media stream for call {} with constraints: {:?}",
            call_id.0, constraints
        );
    ...
    pub async fn start_capture(&self, call_id: CallId) -> Result<()> {
        ...
        if let Some(stream) = streams.get_mut(&call_id) {
            ...
            if let Some(ref mut screen_track) = stream.screen_track {
                self.video_processor
                    .start_screen_capture(screen_track)
                    .await?;
            }
            ...
            let _ = self
                .event_sender
                .send(MediaEvent::CaptureStarted { call_id });
    ...
    pub async fn stop_capture(&self, call_id: CallId) -> Result<()> {
        ...
        if let Some(stream) = streams.get_mut(&call_id) {
            ...
            if let Some(ref mut screen_track) = stream.screen_track {
                self.video_processor
                    .stop_screen_capture(screen_track)
                    .await?;
            }
            ...
            let _ = self
                .event_sender
                .send(MediaEvent::CaptureStopped { call_id });
    ...
    pub async fn adapt_quality(&self, call_id: CallId, metrics: &CallQualityMetrics) -> Result<()> {
        ...
        if let Some(stream) = streams.get_mut(&call_id) {
            ...
            if let Some(ref mut video_track) = stream.video_track {
                self.video_processor
                    .update_settings(video_track, &settings)
                    .await?;
            }

            if let Some(ref mut audio_track) = stream.audio_track {
                self.audio_processor
                    .update_settings(audio_track, &settings)
                    .await?;
            }
    ...
    pub async fn remove_stream(&self, call_id: CallId) -> Result<()> {
        ...
        if streams.remove(&call_id).is_some() {
            ...
            let _ = self
                .event_sender
                .send(MediaEvent::StreamRemoved { call_id });
    ...
    pub async fn switch_audio_device(&self, call_id: CallId, device: AudioDevice) -> Result<()> {
        ...
        {
            self.audio_processor
                .switch_device(audio_track, device)
                .await?;
        }
    ...
    pub async fn switch_video_device(&self, call_id: CallId, device: VideoDevice) -> Result<()> {
        ...
        {
            self.video_processor
                .switch_device(video_track, device)
                .await?;
        }
...
impl AudioProcessor {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        *device_list = devices;
        ...
        info!(
            "Initialized audio processor with {} devices",
            device_list.len()
        );
        Ok(())
    }
    ...
    pub async fn update_settings(
        &self,
        _track: &mut AudioTrack,
        settings: &AdaptationSettings,
    ) -> Result<()> {
        debug!(
            "Updating audio settings: bitrate={}kbps, dtx={}",
            settings.audio_bitrate_kbps, settings.enable_dtx
        );
        Ok(())
    }
...
impl VideoProcessor {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        *device_list = devices;
        ...
        info!(
            "Initialized video processor with {} devices",
            device_list.len()
        );
        Ok(())
    }
    ...
    pub async fn update_settings(
        &self,
        _track: &mut VideoTrack,
        settings: &AdaptationSettings,
    ) -> Result<()> {
        debug!(
            "Updating video settings: {}x{} @{}fps {}kbps",
