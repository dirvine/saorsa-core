...
impl SecureMessaging {
    ...
    pub async fn encrypt_message(&self, message: &RichMessage) -> Result<EncryptedMessage> {
        ...
        let session_key = if let Ok(key) = self.key_exchange.get_session_key(&message.channel_id.0.to_string().into()).await {
            key
        } else {
    ...
    pub async fn create_ephemeral_session(&self, peer: &FourWordAddress) -> Result<EphemeralSession> {
