...
impl MessageComposer {
    ...
    pub fn start_draft(&mut self, channel_id: ChannelId) -> &mut DraftMessage {
        self.drafts
            .entry(channel_id)
            .or_insert_with(|| DraftMessage::new(channel_id))
    }
    ...
    pub fn validate_draft(&self, channel_id: ChannelId) -> Result<()> {
        ...
        let total_size: usize = draft.attachments.iter().map(|a| a.size).sum();
    ...
    pub fn build_message(
        ...
        // Convert attachments
        let attachments = draft.attachments.iter().map(|a| a.data.clone()).collect();
...
impl DraftMessage {
    ...
    fn extract_links(&self) -> Vec<String> {
        fn try_re(p: &str) -> Option<regex::Regex> {
            regex::Regex::new(p).ok()
        }
        let url_regex = try_re(r"https?://[^\s<]+[^<.,:;'!\?\s]")
            .or_else(|| try_re(r"https?://.+"))
            .or_else(|| try_re(r"https?://.*"));
