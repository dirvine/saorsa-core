...
async fn main() -> anyhow::Result<()> {
    ...
    println!("   Loading {} records into optimized storage...", record_count);
    ...
    let store_speedup = legacy_store_time.as_nanos() as f64 / optimized_store_time.as_nanos() as f64;
    println!("   ✅ Legacy: {:?} | Optimized: {:?} | Speedup: {:.2}x", 
             legacy_store_time, optimized_store_time, store_speedup);

    // Benchmark 2: Retrieval operations
    println!("\n2️⃣ Retrieval Operations");
    let test_keys: Vec<&Key> = test_records.iter()
        .step_by(100) // Test every 100th record
        .map(|record| &record.key)
        .collect();

    println!("   Testing {} random retrievals from legacy storage...", test_keys.len());
    let legacy_start = Instant::now();
    ...
    println!("   Testing {} random retrievals from optimized storage...", test_keys.len());
    let optimized_start = Instant::now();
    for key in &test_keys {
    ...
    let test_publishers = vec!["publisher_0", "publisher_25", "publisher_50", "publisher_75", "publisher_99"];
    ...
    for publisher in &test_publishers {
        let records = optimized_storage.get_records_by_publisher(publisher, None).await;
        optimized_results += records.len();
    }
    let optimized_query_time = optimized_start.elapsed();

    let query_speedup = legacy_query_time.as_nanos() as f64 / optimized_query_time.as_nanos() as f64;
    println!("   ✅ Legacy: {:?} ({} results) | Optimized: {:?} ({} results) | Speedup: {:.2}x", 
    ...
    println!("      - Memory usage: {} bytes ({:.2} MB)", stats.memory_usage_bytes, stats.memory_usage_bytes as f64 / 1_048_576.0);
    println!("      - Cache hits: {}, misses: {}", stats.cache_hits, stats.cache_misses);
    println!("      - Hit ratio: {:.1}%", (stats.cache_hits as f64 / (stats.cache_hits + stats.cache_misses) as f64) * 100.0);
    ...
    for i in 0..2000 { // Try to store more than cache can hold
    ...
    println!("      - Cache size: {} (≤ 1000 limit)", bounded_stats.total_records);
    println!("      - Memory usage: {} bytes", bounded_stats.memory_usage_bytes);
    ...
    println!("Publisher queries:    {:.2}x speedup (O(n) → O(1))", query_speedup);
    
    let average_speedup = (store_speedup + get_speedup + query_speedup) / 3.0;
    println!("\n🚀 Average performance improvement: {:.2}x", average_speedup);
